...
def detect(opt, stframe, car, bus, truck, motor, line, fps_rate, class_id):
    ...
    # webcam = source == "0"
    # webcam = source == 0 or str(source).startswith('http') or str(source).startswith('rtsp')

    webcam = source == '0' or str(source).startswith(
        'rtsp') or str(source).startswith('http') or str(source).endswith('.txt')
    sum_fps = 0
    line_pos = line
    save_vid = True
    # initialize deepsort
